...
Combine mr_trash_wheel_df, prof_trash_wheel_df, and gwynnda_df
...
The dataset has 17455 observations and 6 variables. The dataset contains
...
appear in the Zillow Rental Price dataset. These zip codes are 10464,
...
This can occur because there are certain zip codes that either have low
or no rental housing associated with it, so it cannot produce a reliable
ZORI score. This is because some zip codes cover multiple boroughs and
...
JFK airport. Since these locations may not have residential housing
associated with it, these zip codes won’t be included in the Zillow
dataset.
...
filter_combined_zip_zori_df =
  ...
  mutate(
    price_drop = `2020` - `2021` # more positive number, larger drop in price
  ) |> 
...
| zip_code | neighborhood                  | county   |     2020 |     2021 | price_drop |
|---------:|:------------------------------|:---------|---------:|---------:|-----------:|
...
observations. The `price_drop` variable is included to represent the
ZORI difference between 2020 to 2021, by calculating ZORI in 2020 minus
ZORI in 2021. This means the larger and more positive the `price_drop`
value is, the larger the price drop.

The 10 zip codes with the largest drop in price from January 2020 to
January 2021 all reside in Manhattan (New York County), but includes
different neighborhoods within Manhattan. 10007 had the highest rental
price drop between January 2020 and January 2021 of \$912.597. The top
10 price drops ranges from \$684.93 to \$912.597.
