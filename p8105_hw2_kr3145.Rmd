...
observations. The `price_drop` variable is included to represent the ZORI difference between
2020 to 2021, by calculating ZORI in 2020 minus ZORI in 2021. This means the larger and more
positive the `price_drop` value is, the larger the price drop. 

The 10 zip codes with the largest drop in price from January 2020 to January 2021 all reside
in Manhattan (New York County), but includes different neighborhoods within Manhattan. 
...
\$`r round(max(pull(filter_combined_zip_zori_df, price_drop)), 3)`. The top 10 price drops
ranges from \$`r round(filter_combined_zip_zori_df |> slice(10) |> pull(price_drop), 2)` to
\$`r round(max(pull(filter_combined_zip_zori_df, price_drop)), 3)`.
